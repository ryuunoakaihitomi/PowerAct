...
public class ReflectionUtils {
    ...
    private static void removeRestriction() {
        ...
        switch (Build.VERSION.SDK_INT) {
            case R:
                if (debug) DebugLog.d(TAG, "removeRestriction: Create new thread for R");
                new Thread(() -> {
                    ...
                    try (FileOutputStream fos = new FileOutputStream(code)) {
                        ...
                        if (debug) DebugLog.e(TAG, "removeRestriction: ", e);
                    } finally {
                        if (code.exists()) {
                        ...
                        if (debug)
                            DebugLog.d(TAG, "removeRestriction: Have non-restricted reflections!");
                    }
                }).start();
                break;
            case Q:
            case P:
                ...
                break;
            default:
                if (debug) DebugLog.v(TAG, "removeRestriction: no-op");
        }
    }
}
